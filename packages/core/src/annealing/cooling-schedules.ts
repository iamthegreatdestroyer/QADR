...
import type { IAnnealingConfig, ICoolingSchedule } from './types.js';
 ...
 */
export function stepCooling(stepsPerLevel = 1000, dropFactor = 0.8): ICoolingSchedule {
  return (currentTemp: number, iteration: number): number => {
...
export function customCooling(
  ...
  return (_currentTemp: number, iteration: number, config: IAnnealingConfig): number => {
    return tempFunc(iteration, config.initialTemperature, config.finalTemperature);
  };
}
