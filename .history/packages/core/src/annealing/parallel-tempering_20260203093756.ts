...
export class ParallelTempering {
  ...
  public solve(
    ...
    const replicas = this.initializeReplicas(
      temperatures,
      hamiltonian,
      options.initialSolutions
    );
    ...
    for (
      let iteration = 0;
      iteration < this.config.iterationsPerReplica;
      iteration++
    ) {
  ...
  private generateTemperatureLadder(): number[] {
    const { numReplicas, minTemperature, maxTemperature, temperatureDistribution } =
      this.config;
  ...
  private localMove(replica: IReplica, hamiltonian: Hamiltonian): void {
    ...
    const accept =
      deltaE < 0 ||
      this.rng() < Math.exp(-deltaE / replica.temperature);
  ...
  public static estimateOptimalSpacing(
    ...
    const tempRatio = 1.2;
    const numReplicas = Math.ceil(
      Math.log(maxTemp / minTemp) / Math.log(tempRatio)
    );
