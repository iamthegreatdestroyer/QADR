...
import type { IQUBOMatrix, IQUBOSolution } from '../qubo/types.js';
...
import { exponentialCooling } from './cooling-schedules.js';
...
export class SimulatedAnnealing {
  ...
  public solve(
    ...
    for (let iteration = 0; iteration < this.config.maxIterations; iteration++) {
      ...
      for (let step = 0; step < this.config.iterationsPerTemperature; step++) {
        ...
        const accept =
          deltaE < 0 ||
          this.rng() < Math.exp(-deltaE / temperature);
      ...
      // Restart if stuck
      if (
        this.config.enableRestarts &&
        iterationsSinceImprovement > this.config.restartThreshold
      ) {
      ...
      // Cool down
      temperature = this.config.coolingSchedule(
        temperature,
        iteration,
        this.config
      );
      ...
      if (iteration % progressInterval === 0) {
        const acceptanceRate =
          acceptedMoves + rejectedMoves > 0
            ? acceptedMoves / (acceptedMoves + rejectedMoves)
            : 0;
   ...
   */
  public static estimateInitialTemperature(
    matrix: IQUBOMatrix,
    samples = 1000
  ): number {
    const hamiltonian = new Hamiltonian(matrix);
