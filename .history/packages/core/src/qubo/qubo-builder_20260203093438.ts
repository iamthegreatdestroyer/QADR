...
import type { IDependencySpec, IDependencyNode } from '../types.js';
import type {
  IQUBOProblem,
  IQUBOVariable,
  IQUBOMatrix,
...
import { QUBOConstraintType, DEFAULT_QUBO_CONFIG, quadraticKey } from './types.js';
...
export class QUBOBuilder {
  ...
  private createVariables(input: IQUBOBuilderInput): {
    ...
    for (const [packageName, node] of input.packageVersions) {
      for (const version of node.versions) {
        ...
        const recencyBias =
          -this.config.recencyBonus * (node.versions.length - 1 - recencyIndex);
  ...
  private buildConstraints(
    ...
    constraints.push(
      ...this.buildDependencyConstraints(input, variableIndex)
    );
  ...
  private computeStats(
    ...
    const avgVersionsPerPackage =
      packageCount > 0 ? variableCount / packageCount : 0;
